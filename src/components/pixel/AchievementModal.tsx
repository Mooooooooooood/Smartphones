 ...
 * "how to unlock" copy, and a progress bar toward the target. Tap-to-open from
 * the Profile achievement shelf.
 */
export default function AchievementModal({ achievement, onClose }: { achievement: AchievementView | null; onClose: () => void }) {
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-[rgba(4,6,20,0.7)] p-4 backdrop-blur-sm" role="dialog" aria-modal="true" onClick={onClose}>
      <div className="w-full max-w-[300px]" onClick={(e) => e.stopPropagation()}>
        ...
        >
          ...
          <button type="button" onClick={onClose} className="px-btn mt-4 w-full">{a.unlocked ? "Nice!" : "Got it"}</button>
        </div>
      </div>
    </div>
  );
